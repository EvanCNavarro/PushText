...
struct SettingsStoreTests {
    ...
    func savedValueRoundTrips() {
        ...
        store.purge()
        store.save(AppSettings(cleanupEnabled: true, hotkeyKeyCode: HotkeyBinding.rightOption.keyCode))
...
extension SettingsStoreTests {
    ...
    func hotkeyRoundTrips() {
        ...
        store.save(AppSettings(cleanupEnabled: false,
                               hotkeyKeyCode: HotkeyBinding.rightCommand.keyCode))
    ...
    func bothFieldsPersist() {
        ...
        store.save(AppSettings(cleanupEnabled: true,
                               hotkeyKeyCode: HotkeyBinding.rightShift.keyCode))
    ...
    func unknownCodeFallsBack() {
        let settings = AppSettings(cleanupEnabled: false, hotkeyKeyCode: 0x7FFF)
