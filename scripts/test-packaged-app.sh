...
# Bundle.module must stay DEBUG-only (TRAP-4). One definition, in .engine/checks, so CI runs it
# on every PR as well as here - it used to live only in this file, which needs a built app.
"$ROOT/.engine/checks/bundle-module-guarded.sh" || fail "Bundle.module used outside a DEBUG guard"
