...
struct PushTextApp: App {
    ...
    init() {
        ...
        Self.installProbes(on: launchDelegate, model: model, actions: actions)
    }
    ...
    /// Every launch-time probe, in one place - see `PushTextApp+Probes.swift`.
    private static func installProbes(on launchDelegate: LaunchDelegate,
                                      model: AppModel,
                                      actions: AppActions) {
        installHUDProbeIfRequested(on: launchDelegate)
        installMenuProbeIfRequested(on: launchDelegate, model: model, actions: actions)
        LayeringProbe.install(on: launchDelegate, actions: actions)
    }
